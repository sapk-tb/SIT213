...
function get-teb-by-snr-with-multi {
    ...
    MULTI=${5:-""}
    OTHER=${6:-""}
    ARGS="-form $FORM -mess $NB_SYM -nbEch $NB_ECH -snr $SNR -ampl -1 1 $MULTI $OTHER"
    ./simulateur $ARGS | grep TEB | cut -d":" -f2 | tr -d " "
}
...
function generate-teb-by-snr-with-multi {
    ...
    do
        TEB_T1=$(get-teb-by-snr-with-multi $snr $FORM $4 $5 " " "$OTHER")
        TEB_T2=$(get-teb-by-snr-with-multi $snr $FORM $4 $5 "-ti 1 12 1" "$OTHER")
        TEB_T3=$(get-teb-by-snr-with-multi $snr $FORM $4 $5 "-ti 1 60 1" "$OTHER")
	TEB_T4=$(get-teb-by-snr-with-multi $snr $FORM $4 $5 "-ti 1 45 0.7 -ti 2 60 0.4" "$OTHER")
	...
	for nbSym in 100 10000 1000000 10000000
		...
		for nbEch in 3 5 10 15 30 60
			...
			for form in "RZ" "NRZ" "NRZT"
				do
	                        for correction in "-noMultiCorrection" ""
        	                        do
					OUTPUT="$BASE_DIR/data/csv/teb-by-snr-with-multi$correction-$form-$nbSym-$nbEch.csv"
		    			echo "SNR,TEB_T1,TEB_T2,TEB_T3,TEB_T4" > $OUTPUT                
				
	#				echo "generate-teb-by-snr-with-multi -60 -30 5 $nbSym $nbEch $form $correction"
	#	                	time generate-teb-by-snr-with-multi  -60 -30 5 $nbSym $nbEch $form "$correction" >> $OUTPUT
		
	#				echo "generate-teb-by-snr-with-multi -28 -21 2 $nbSym $nbEch $form $correction"
	#	    			time generate-teb-by-snr-with-multi  -28 -21 2 $nbSym $nbEch $form "$correction" >> $OUTPUT
		
					echo "generate-teb-by-snr-with-multi -20 -12 2 $nbSym $nbEch $form $correction"
		    			time generate-teb-by-snr-with-multi  -20 -12 2 $nbSym $nbEch $form "$correction" >> $OUTPUT
		
					echo "generate-teb-by-snr-with-multi -10 -4 1 $nbSym $nbEch $form $correction"
		    			time generate-teb-by-snr-with-multi  -10 -4 1 $nbSym $nbEch $form "$correction" >> $OUTPUT
		
					echo "generate-teb-by-snr-with-multi -3 4.8 0.2 $nbSym $nbEch $form $correction"
			    		time generate-teb-by-snr-with-multi  -3 4.8 0.2 $nbSym $nbEch $form "$correction" >> $OUTPUT
		
					echo "generate-teb-by-snr-with-multi 5 10 0.5 $nbSym $nbEch $form $correction"
			    		time generate-teb-by-snr-with-multi  5 10 0.5 $nbSym $nbEch $form "$correction" >> $OUTPUT
		    		done
			done
	    	done
	done
